<template>
  <div>
	...
	<div id="Overview" class="px-6">
		<h3 class="font-semibold text-sidebar-title">Фото ({{this.selectedDataItem.photo.length || 'none'}})</h3>

	  	<div class="mt-2 mb-6 w-full h-[132px]">
		  <Carousel  :breakpoints="breakpoints">
			...
			@click="ShowModal">
			  <div class="mx-0.5 p-0.5 bg-gray-50 rounded-lg">
	  ...
	  <h3 class="font-semibold text-sidebar-title">Загальний стан</h3>

	  <div class="my-2 w-full h-[134px] bg-light-blue rounded-lg p-6 flex flex-nowrap">
			  <img
...
<!--	#region  Build status-->
	  <div>
		<div class="text-base py-4 shadow-2cs">
			<div class="mb-2 flex flex-nowrap">
				<svg :class="markerReports.buildingCondition === `Неушкоджена` ? 'fill-safety-green' : 'fill-dangerous-red'"
				...
				<p class="grow font-semibold text-overview-item my-auto uppercase"
					:class="markerReports.buildingCondition === `Неушкоджена` ? 'text-safety-green' : 'text-dangerous-red'">
				  {{markerReports.buildingCondition}}
				</p>
				<p class="text-base-grey text-overview-item my-auto font-normal">
				  {{(new Date(this.selectedDataItem.lastUpdate)).toLocaleString()}}
				</p>
			</div>
		  	<div class="min-h-min bg-gray-light-200 text-base-grey text-overview-item p-2 rounded-sm">
		...
		<div class="flex flex-nowrap text-base py-4 shadow-2cs">
		  <svg :class="markerReports.electricity === `Стабільна` ? 'fill-safety-green' : 'fill-dangerous-red'"
		  ...
		  <p class="grow font-semibold text-overview-item my-auto uppercase"
			 :class="markerReports.electricity === `Стабільна` ? 'text-safety-green' : 'text-dangerous-red'">
		...
		<div class="flex flex-nowrap text-base py-4 shadow-2cs">
		  <svg :class="markerReports.carEntrance === `Доступне` ? 'fill-safety-green' : 'fill-dangerous-red'"
		  ...
		  <p class="grow font-semibold text-overview-item my-auto uppercase"
			 :class="markerReports.carEntrance === `Доступне` ? 'text-safety-green' : 'text-dangerous-red'">
		...
		<div class="flex flex-nowrap text-base py-4 shadow-2cs">
			<p class="mr-1.5 my-auto w-6">
				<SVG_water :class="markerReports.water === `Стабільна` ? 'fill-safety-green' : 'fill-dangerous-red'"/>
			</p>
		  <p class="grow font-semibold text-overview-item my-auto uppercase"
			 :class="markerReports.water === `Стабільна` ? 'text-safety-green' : 'text-dangerous-red'">
			{{markerReports.water}}
		  </p>
		  <p class="text-base-grey text-overview-item my-auto font-normal">
			{{(new Date(this.selectedDataItem.lastUpdate)).toLocaleString()}}
		...
		<div class="flex flex-nowrap text-base py-4 shadow-2cs">
		  <svg :class="markerReports.fuelStation === `відчинено` ? 'fill-safety-green' : 'fill-dangerous-red'"
		  ...
		  <p class="grow font-semibold text-overview-item my-auto uppercase"
			 :class="markerReports.fuelStation === `відчинено` ? 'text-safety-green' : 'text-dangerous-red'">
			{{markerReports.fuelStation}}
			<span v-if="markerReports.fuelStation !== `none`" class="text-base-grey text-overview-item font-semibold">
		...
		<div class="flex flex-nowrap text-base py-4 shadow-2cs">
			<p class="mr-1.5 my-auto w-6">
				<SVG_fuel_station :class="markerReports.hospital === `відчинено` ? 'fill-safety-green' : 'fill-dangerous-red'"/>
			</p>


		  <p class="grow font-semibold text-overview-item my-auto uppercase"
			 :class="markerReports.hospital === `відчинено` ? 'text-safety-green' : 'text-dangerous-red'">
			{{markerReports.hospital}}
			<span v-if="markerReports.hospital !== `зачинено`" class="text-base-grey text-overview-item font-semibold">
...
<!--	  #region Form-->
	<div class="my-6">
		<label for="issueMessage" class="text-grey text-overview-item font-normal">
      Буде корисно, якщо ви повідомите нам через цю форму про будь-які проблеми, пов’язані з використанням нашого сервісу.
		...
		<div class="h-min">
		  	...
		  	<button @click="this.Show(issueMessage)"
				...
				:class="this.isDisabled ? `bg-gray-light-200 text-base-grey`:
					`bg-blue-200 text-base-blue hover:border-base-blue`"
				class="block border rounded-lg px-8 py-1.5">
					Відправити
		  	</button>
		</div>
	  </div>
<!--	  #endregion-->
	...
	<div id="contact" class="p-6 bg-gray-light-100">
		<div class="font-semibold text-overview-item">Урядові контакти</div>
		<div class="flex flex-wrap justify-between py-1 shadow-2cs mt-2">
			<p class="text-overview-item">Служба безпеки України</p>
	  ...
	  <div class="flex flex-wrap justify-between py-1">
		  <p class="text-overview-item">Державна прикордонна служба України</p>
		  <p class="text-right font-bold text-base-blue">
